...
class calc_fncs:
    ...
    def get_data_TW_regressions(df,regression_var):
        """
        """
    ...
    def get_projections(df, ellipse_mode_selector, power):
        ...
        discorida_regressions = calc_fncs.get_data_TW_regressions(df,regression_var)  # get regressions
    ...
    def plot_TW(df, xlim_start, xlim_stop, ylim_start, ylim_stop, label_toggle, ellipse_mode_selector, power):
        ...
        disc_regressions = calc_fncs.get_data_TW_regressions(df,regression_var)
    ...
    def plot_weth(df, xlim_start_weth, xlim_stop_weth, ylim_start_weth, ylim_stop_weth, label_toggle, ellipse_mode_selector, power):
        """
        """
        plt.style.use('seaborn-colorblind')
        fig = Figure(figsize=(10, 14))
        ...
        if ellipse_mode_selector == 'Point Estimates':
            ax.errorbar(df['207Pb/235U'], 1/df['238U/206Pb'], yerr=df['206/238 Reg. err'],xerr=df['207/235 Reg. err'], fmt='none', ecolor='k', elinewidth=0.5)
            ax.plot(df['207Pb/235U'], 1/df['238U/206Pb'], 'kd')
            ax.plot(x_Weth, y_Weth, 'k', lw=1)
            if 'Concordia' in label_toggle:
                for x, y, t in zip(df['207Pb/235U'],df['238U/206Pb'], df['SampleLabel']):
                    label = t
                    ax.annotate(label, (x, 1/y),textcoords="offset points",  xytext=(0, 10),ha='center',fontsize=8)
            ...
            for i in df.SampleLabel.unique():
                ...
                ax.add_artist(ell)
            ax.plot(x_Weth, y_Weth, 'k', lw=1)
        ax.set_xlabel('$^{207}$Pb / $^{235}$U', fontsize=12)
        ax.set_ylabel('[$^{206}$Pb / $^{238}$Pb', fontsize=12)
        ax.tick_params(axis='both', labelsize=10)
        ax.set_ylim(ylim_start_weth, ylim_stop_weth)
        ax.set_xlim(xlim_start_weth, xlim_stop_weth)
    ...
    def correct_sample_ages(df, std, std_txt, ThU_zrn, ThU_magma, Pb_Th_std_crct_selector, regression_selector, ellipse_mode_selector, power, UTh_std_norm):
        ...
        if ellipse_mode_selector == 'Point Estimates':
            ...
            frac_factor, frac_factor_207, tims_age, tims_error, tims_age_207, tims_error_207, avg_std_age, avg_std_age_Thcrct, avg_std_age_207, avg_std_ratio, avg_std_ratio_Thcrct, avg_std_ratio_207, avg_reg_err, avg_reg_err_207, UTh_std, UTh_std_m = \
                calc_fncs.get_standard_fracfctr(std, std_txt, Pb_Th_std_crct_selector, regression_selector, ellipse_mode_selector, power)
    ...
    def get_standard_fracfctr(std, std_txt, Pb_Th_std_crct_selector, regression_selector, ellipse_mode_selector, power):
        ...
        avg_std_age, avg_std_age_Thcrct, avg_std_age_207, avg_std_ratio, avg_std_ratio_Thcrct, avg_std_ratio_207, avg_reg_err, avg_reg_err_207, UTh_std, UTh_std_m = \
            calc_fncs.correct_standard_ages(std, std_txt, Pb_Th_std_crct_selector, regression_selector, ellipse_mode_selector, power)
...
class finalize_ages(param.Parameterized):
    ...
    y_axis_Weth = param.Range(default=(0.0, 1.0), bounds=(0.0, 1.2))  # for y-axis on Wetherhill
    x_axis_Weth = param.Range(default=(0.0, 40.0), bounds=(0.0, 90.0))  # for y-axis on Wetherhill
    ...
    @pn.depends('input_data', 'input_data_ellipse', 'text_sample_selector', 'y_axis_TW', 'label_toggle',
                'x_axis_TW_min', 'x_axis_TW_max', 'x_axis_Weth', 'y_axis_Weth','ellipse_mode_selector', 'power')
...
grid_layout[0, 0] = pn.Column(reduce_ages.call_Concordia)
