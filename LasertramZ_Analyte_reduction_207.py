...
class calc_fncs:
    ...
    def get_regressions(data,regression_buttons,ablation_start_true,force_exp_params,joggle_params,aguess,bguess,cguess,aguess_207,bguess_207,cguess_207):
        """
        ...
        if 'Exp. Regression' in regression_buttons:
            ...
            if force_exp_params is True:
                if joggle_params is True:
                    ...
                    popt_207,pcov_207 = curve_fit(exp_func,data['Time_s'],data['207Pb/235U'],p0=(aguess_207,bguess_207,cguess_207))
                elif joggle_params is False:
                    popt,pcov = curve_fit(exp_func,data['Time_s'],data['206Pb/238U'],p0=(0.1,0.05,0.3))
                    popt_207,pcov_207 = curve_fit(exp_func,data['Time_s'],data['207Pb/235U'],p0=(0.01,0.05,0.001))
            else:
                popt,pcov = curve_fit(lambda t, a, b, c: exp_func, data['Time_s'],data['206Pb/238U'])
                popt_207,pcov_207 = curve_fit(lambda t, a, b, c: exp_func, data['Time_s'],data['207Pb/235U'])
            ...
            sigmaexp_207 = np.sqrt(sum_sq_resid_207/(len(data['207Pb/235U'])-3)) # denominator = d.f. = n-#params
    ...
    def get_approved(data,bckgrnd_start_input,bckgrnd_stop_input,
                    ...
                    regression_buttons,ellipsemode_selector,force_exp_params,joggle_params,aguess,bguess,cguess,aguess_207,bguess_207,cguess_207,
        ...
        if counts_mode == 'Total Counts':
            ...
            data_ratios_reg,data_stats_reg = calc_fncs.get_regressions(data_approved_ratio,regression_buttons,ablation_start_true,force_exp_params,joggle_params,aguess,bguess,cguess,aguess_207,bguess_207,cguess_207) # get estimated regression intercepts, background subtracted ratios, regression statistics, and SE's of the ratios3
...
class plots(calc_fncs):
    ...
    def ratio_plot(data,ratio_buttons,regression_buttons,
                   ...
                   ratio_plot_ylim_slider_min,ratio_plot_ylim_slider_max,force_exp_params,joggle_params,aguess,bguess,cguess,aguess_207,bguess_207,cguess_207):
        """
        ...
        regressions,stats = calc_fncs.get_regressions(data_to_regress,regression_buttons,ablation_start_true,force_exp_params,joggle_params,aguess,bguess,cguess,aguess_207,bguess_207,cguess_207)
    ...
    def residuals_plot(data,regression_buttons,start_ablation,stop_ablation,ablation_start_true,force_exp_params,joggle_params,aguess,bguess,cguess,aguess_207,bguess_207,cguess_207):
        plt.style.use('seaborn-colorblind') # use colorscheme that is viewable by colorblind folks
        fig,ax = plt.subplots(2,1,figsize=(8,6)) # create a figure that is 6Wx3H
        ...
        fitted,fitted207,residuals,residuals207 = calc_fncs.get_regressions(data_to_regress,regression_buttons,ablation_start_true,force_exp_params,joggle_params,aguess,bguess,cguess,aguess_207,bguess_207,cguess_207) # get fitted regression values, regression names, and residuals
        # plot the residuals for the regressions and the fitted value
        for j in range(0,len(regression_buttons)):
            ax[0].plot(fitted[j],residuals[j],'o',mec='k',label='{}'.format(regression_buttons[j]))
            ax[1].plot(fitted207[j],residuals207[j],'o',mec='k',label='{}'.format(regression_buttons[j]))
        ax[0].plot([min(fitted[0]),max(fitted[0])],[0,0],'-k',lw=0.5)
        ax[1].plot([min(fitted207[0]),max(fitted207[0])],[0,0],'-k',lw=0.5)
        
        ax[0].legend(loc='upper right',fontsize=8) # get the legend
        ax[0].set_xlabel('Fitted Value 206/238',fontsize=10) # xlabel
        ax[0].set_ylabel('Residuals 206/238',fontsize=10) # ylabel
        ax[0].tick_params(axis='both',labelsize=8) # plot aesthetics
        ax[1].legend(loc='upper right',fontsize=8) # get the legend
        ax[1].set_xlabel('Fitted Value 207/235',fontsize=10) # xlabel
        ax[1].set_ylabel('Residuals 207/235',fontsize=10) # ylabel
        ax[1].tick_params(axis='both',labelsize=8) # plot aesthetics
...
class make_plots(param.Parameterized):
    ...
    cguess_207 = param.Number(default=0)
    ...
    @pn.depends('input_data','ratio_buttons','regression_buttons',
                ...
                'sample_subset',
                'ratio_plot_ylim_min','ratio_plot_ylim_max','force_exp_params','joggle_params','aguess','bguess','cguess','aguess_207','bguess_207','cguess_207')
    def call_ratio_plot(self):
        if self.sample_subset is not None:
            ...
            return plots.ratio_plot(data_toplot,self.ratio_buttons,self.regression_buttons,self.background_slider[0],self.background_slider[1],
                              ...
                              self.ratio_plot_ylim_min,self.ratio_plot_ylim_max,self.force_exp_params,self.joggle_params,self.aguess,self.bguess,self.cguess,self.aguess_207,self.bguess_207,self.cguess_207)
    
    @pn.depends('input_data','regression_buttons','ablation_slider','sample_subset','ablation_start_true','force_exp_params','joggle_params','aguess','bguess','cguess','aguess_207','bguess_207','cguess_207')
    def call_residuals_plot(self):
        if self.sample_subset is not None:
            ...
            return plots.residuals_plot(data_toplot,self.regression_buttons,self.ablation_slider[0],self.ablation_slider[1],self.ablation_start_true,self.force_exp_params,self.joggle_params,
                                        self.aguess,self.bguess,self.cguess,self.aguess_207,self.bguess_207,self.cguess_207)
    ...
    def get_regression_stats(self):
        if self.sample_subset is not None:
            ...
            stats_markdown = pn.Column(divider1,r2_1,r2_2,r2_exp,SE_1stper,SE_2ndper,SE_expper,divider2,SE_1stper_207,SE_2ndper_207,SE_expper_207)
